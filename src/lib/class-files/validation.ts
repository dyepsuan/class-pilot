...
export const MAX_CLASS_FILE_SIZE = 25 * 1024 * 1024;
export const MAX_CLASS_FILE_FILENAME_LENGTH = 255;
...
export const CLASS_FILE_INPUT_ACCEPT = Object.keys(MIME_TYPES_BY_EXTENSION)
  .map((extension) => `.${extension}`)
  .join(",");

export type ClassFileUploadCandidate = {
  name: string;
  size: number;
  type?: string;
};

export type ClassFileValidationErrorCode =
  | "INVALID_FILENAME"
  | "EMPTY_FILE"
  | "FILE_TOO_LARGE"
  | "INVALID_FILE_SIZE"
...
export type ClassFileValidationResult =
  | {
      ok: true;
      extension: string;
      mimeType: string;
  ...
  | {
      ok: false;
      code: ClassFileValidationErrorCode;
      message: string;
    };
...
export function validateClassFileUpload(
  file: ClassFileUploadCandidate
): ClassFileValidationResult {
  const filename = file.name.trim();

  if (
    filename.length === 0 ||
    filename.length > MAX_CLASS_FILE_FILENAME_LENGTH ||
    /[\u0000-\u001f\u007f]/u.test(filename) ||
    /[\\/]/u.test(filename)
  ) {
    return {
  ...
  if (file.size > MAX_CLASS_FILE_SIZE) {
    return {
      ok: false,
      code: "FILE_TOO_LARGE",
  ...
  const allowedMimeTypes = extension && Object.hasOwn(MIME_TYPES_BY_EXTENSION, extension)
  ...
  const suppliedMimeType = file.type?.split(";")[0].trim().toLowerCase() ?? "";
  ...
  return {
    ok: true,
    extension,
    mimeType: !suppliedMimeType || GENERIC_MIME_TYPES.has(suppliedMimeType)
      ? allowedMimeTypes[0]
      : suppliedMimeType,
  };
}

export function getSafeClassFileDownloadMimeType(mimeType: string): string {
